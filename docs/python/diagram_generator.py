...
from utils import get_qname, get_label, get_id, fmt_title, get_all_class_superclasses, is_refined_property, collect_list, get_class_expression_str, get_ontology_for_uri, insert_spaces, get_leaf_classes, get_property_info, collect_oneOf, term_page_link_from_diagrams
...
def add_class_expression_node(graph, g: Graph, expr, ns: str, prefix_map: dict, global_all_classes: set, ns_to_ontology: dict, abstract_map: dict, created: set, is_superclass: bool = False, in_associated_cluster: bool = False, enum_members: list = None, enum_name: str = None) -> tuple:
    ...
    if isinstance(expr, URIRef):
        ...
        url = term_page_link_from_diagrams(label) if ':' not in qname and qname in global_all_classes else None
        label = get_label(g, expr)
...
def generate_diagram(g: Graph, cls: URIRef, cls_name: str, cls_id: str, ns: str, global_all_classes: set, abstract_map: dict, ofn_path: str, errors: list, prefix_map: dict, ontology_name: str, ns_to_ontology: dict):
    ...
    with dot.subgraph() as main_group:
        ...
        for restriction in g.objects(cls, RDFS.subClassOf):
            if (restriction, RDF.type, OWL.Restriction) in g:
                ...
                if base_prop and (base_prop, RDF.type, OWL.DatatypeProperty) in g:
                    prop_label = g.value(base_prop, RDFS.label)
                    display_name = str(prop_label) if prop_label else prop_name
                    ...
                    value_str = None
                    if has_value:
                        value_str = str(has_value)
                        if isinstance(has_value, Literal) and has_value.datatype:
                            range_name = get_class_expression_str(g, has_value.datatype, ns, prefix_map)
                        stereotype = "hasValue" if not stereotype else f"{stereotype}, hasValue"
                        restrictions.append("value")  # Optional: Indicate value restriction
                    restriction_str = f"«{', '.join(restrictions)}»" if restrictions else ""
                    data_props[prop_name].append((restriction_str, range_name, stereotype, value_str, display_name))
        ...
        for prop_name, restrictions in sorted(data_props.items()):
            ...
            prop_values = set()
            display_names = set()
            for restriction_str, range_name, stereotype, value_str, display_name in restrictions:
                if restriction_str:
                ...
                if value_str:
                    prop_values.add(value_str)
                if display_name:
                    display_names.add(display_name)
            range_name = range_names.pop() if range_names else "string"
            prop_value = prop_values.pop() if prop_values else ""
            display_name = display_names.pop() if display_names else prop_name
...
#            stereotype_label = ", ".join(sorted(set(s for s in stereotypes))) if stereotypes else ""
            if prop_value:
                attribute = f"{display_name} = {prop_value}"
            else:
                attribute = f"{display_name}: {range_name}"
...
#                attribute = f"{attribute} «{', '.join(labels)}»"
        ...
        main_group.node(
            cls_id,
            main_label,
            URL=term_page_link_from_diagrams(cls_name) if cls_name in global_all_classes else None,
            margin="0"
        )
        ...
        if (restriction, RDF.type, OWL.Restriction) in g:
            ...
            if base_prop and (base_prop, RDF.type, OWL.ObjectProperty) in g:
                ...
                if target_expr and label_parts:
                    ...
                    prop_label = g.value(base_prop, RDFS.label)
                    prop_name = str(prop_label) if prop_label else prop_name
